...
def main():
    ...
    hist = autoencoder.fit(train_data, train_data, epochs=20, batch_size=512, validation_split=0.2)
    stmts = hist.history
    ...
    loss = losses.mean_absolute_error(reconstructed, new_data)

    # Calculate threshold for anomaly detection (using train data)
    train_reconstructed = autoencoder(train_data)
    train_loss = losses.mean_absolute_error(train_reconstructed, train_data)
    ...
    if anomalies.numpy().any():
       return stmts, ("The given data indicates some abnormality in ECG kindly consult concerned specialist.")
    
    return stmts, ("The given data is normal.")