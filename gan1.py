...
def train_gan(generator, discriminator, gan, epochs, batch_size):
    ...
    for epoch in range(epochs):
        ...
        # Print progress 
        stmt = (f'{epoch} [D loss: {d_loss[0]} | D accuracy: {100*d_loss[1]}] [G loss: {g_loss}]')
        # print(f"{epoch} [D loss: {d_loss[0]} | D accuracy: {100*d_loss[1]}] [G loss: {g_loss}]")
        plt.plot(np.arange(140), real_samples[0])
        plt.grid()
        plt.title(' ECG')
        plt.show()
        actual_res.append(stmt)
    return actual_res


def main():
# Train the GAN
    res = train_gan(generator, discriminator, gan, epochs=10, batch_size=64)
    ...
    stmt = (f'Classifier Accuracy: {accuracy * 100:.2f}%')
    res.append(stmt)
    return res, repo